...
function dmaCell(ltp: number | null, distPct: number | null | undefined) {
  ...
  const color = distPct >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
  return (
...
function highCell(n: number | null | undefined) {
  ...
  const color = n <= 3 ? 'text-green-600 dark:text-green-400' : n <= 8 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'
...
export default function CandidateRow({ candidate, onOrderPlaced }: Props) {
  ...
  const ltpColor = (candidate.pct_change_today ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'

  return (
    <>
      <tr
        className="border-b border-gray-200 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800/60 cursor-pointer transition-colors"
        onClick={() => navigate(`/candidate/${candidate.symbol}`)}
      >
        <td className="py-2.5 pl-3 pr-4 font-bold text-gray-900 dark:text-white whitespace-nowrap text-center">
          {candidate.symbol}
          {candidate.segment === 'ETF' && (
            <span className="ml-1 text-xs text-blue-600 dark:text-blue-400 border border-blue-800 rounded px-1">ETF</span>
          )}
        </td>
        <td className="py-2.5 pr-4 font-mono text-center whitespace-nowrap">
          <span className="text-gray-900 dark:text-white">{fmt(candidate.ltp, 2)}</span>
        ...
        <td className="py-2.5 pr-4 font-mono text-center whitespace-nowrap">
          <span className="text-gray-700 dark:text-gray-300">{fmt(candidate.high_20d, 2)}</span>
