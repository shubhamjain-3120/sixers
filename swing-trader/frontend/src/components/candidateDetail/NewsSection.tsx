...
const CLS_STYLES: Record<string, { bg: string; text: string; short: string }> = {
  NOISE:                { bg: 'bg-blue-100 dark:bg-blue-900/50',   text: 'text-blue-700 dark:text-blue-300',   short: 'N' },
  FUNDAMENTAL_NEGATIVE: { bg: 'bg-red-100 dark:bg-red-900/50',    text: 'text-red-700 dark:text-red-300',    short: 'F−' },
  FUNDAMENTAL_POSITIVE: { bg: 'bg-green-100 dark:bg-green-900/50',  text: 'text-green-700 dark:text-green-300',  short: 'F+' },
  IRRELEVANT:           { bg: 'bg-gray-100 dark:bg-gray-800',       text: 'text-gray-500',   short: 'IR' },
}

function HeadlineBadge({ cls }: { cls: string }) {
  const s = CLS_STYLES[cls] ?? { bg: 'bg-gray-100 dark:bg-gray-800', text: 'text-gray-600 dark:text-gray-400', short: '?' }
  return (
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-4 mb-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">What's moving the stock</h2>
        <button
          onClick={onReanalyze}
          disabled={classifying}
          className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 disabled:text-gray-600 border border-blue-300 dark:border-blue-900 hover:border-blue-400 dark:hover:border-blue-700 rounded px-2 py-1"
        >
      ...
      {detail.news_verdict ? (
        detail.llm_summary && detail.llm_summary.trim() ? (
          <p className="text-sm text-gray-800 dark:text-gray-200 leading-relaxed">{detail.llm_summary}</p>
        ) : (
            ...
            {(detail.news_headlines ?? []).map(h => (
              <div key={h.idx} className="flex gap-3 text-sm">
                ...
                <div className="min-w-0">
                  <p className="text-gray-700 dark:text-gray-300 leading-snug">{h.headline}</p>
