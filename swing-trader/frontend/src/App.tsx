...
function Nav() {
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `px-3 py-2 rounded text-sm transition-colors ${isActive ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-white'}`
  ...
  return (
    <nav className="border-b border-gray-800 bg-gray-950 sticky top-0 z-10">
      <div className="max-w-6xl mx-auto px-4 flex items-center gap-1 h-12">
        <span className="text-white font-bold mr-4 text-sm">📈 SwingTrader</span>
